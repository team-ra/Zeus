...
// basted on information at http://www.cplusplus.com/reference/cstdio/vsnprintf/
...
// styles create clutter, this makes it harder to read. by putting our styles here our functions will be easer to read
...
void robot_gui(void) {
	...
	//
	lv_obj_t *tv = lv_tabview_create(g_tv_region, NULL);
	lv_obj_set_size(tv, lv_obj_get_width(g_tv_region),
			lv_obj_get_height(g_tv_region));
	set_tv_style(tv);
	...
	lv_label_set_text(g_sb_label, "no auton selected");
